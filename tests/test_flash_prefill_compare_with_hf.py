...
class FlashPrefillCompareWithHFTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.config = Config(
        ...
        # 当前要验证的组合：统一语义 flash attention 主链
...
def main():
    parser = argparse.ArgumentParser(description="Compare flash-prefill + flash-decode outputs with HF.")
    parser.add_argument("-k", "--keyword", default=None)
    args = parser.parse_args()

    loader = unittest.TestLoader()
    suite = loader.loadTestsFromTestCase(FlashPrefillCompareWithHFTest)
